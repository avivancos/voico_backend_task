...
async def test_sql_injection_in_notes_is_stored_verbatim_not_executed(client, make_call):
    victim = await make_call(status=CallStatus.in_progress)
    bystander = await make_call(status=CallStatus.in_progress)
...
async def test_no_mass_assignment_via_notes_endpoint(client, make_call):
    call = await make_call(status=CallStatus.in_progress)  # status=in_progress, summary=None
...
async def test_notes_endpoint_rejects_other_methods(client, make_call):
...
async def test_oversize_notes_is_rejected_before_persistence(client, make_call):
    call = await make_call(status=CallStatus.in_progress)
...
async def test_hostile_unicode_round_trips_without_corruption(client, make_call):
    call = await make_call(status=CallStatus.in_progress)
...
async def test_sql_injection_in_caller_name_filter_is_literal_not_executed(client, make_call):
    await make_call(status=CallStatus.in_progress, caller_name="Alice")
    await make_call(status=CallStatus.in_progress, caller_name="Bob")
...
async def test_like_wildcards_in_caller_name_cannot_broaden_the_match(client, make_call):
    ...
    underscore = await make_call(status=CallStatus.in_progress, caller_name="a_b")
    await make_call(status=CallStatus.in_progress, caller_name="axb")
    await make_call(status=CallStatus.in_progress, caller_name="ab")
...
async def test_phone_filter_injection_is_neutralized_by_digit_normalization(client, make_call):
    row = await make_call(status=CallStatus.in_progress, phone_number="+1 (555) 201-4832")
...
async def test_no_hostile_list_query_param_yields_a_500(client, make_call):
